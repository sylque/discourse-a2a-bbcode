...
export function setup(helper) {
  ...
  helper.registerPlugin(md => {
    ...
    // href: link to follow => mandatory if no user
    ...
    // services: coma-separated list of services to be displayed + "plus" to display a plus button. See the full list at https://www.addtoany.com/services/. By default, service is "twitter,facebook,plus".
    ...
    md.inline.bbcode.ruler.push('abbshare', {
      ...
      replace: function(state, tagInfo, content) {
        ...
        const urlHtml = tagInfo.attrs.url
          ...
          : ''
        const servicesHtml = (tagInfo.attrs.services || 'twitter,facebook,plus')
          .split(',')
          .map(s =>
            s === 'plus'
              ? '<a class="a2a_dd" href="https://www.addtoany.com/share">plus</a>'
              : `<a class="a2a_button_${s}">${s}</a>`
          )
          .join('\n')
        // The a2a_target class is required to use the "target" AddToAny argument
        token.content = `
